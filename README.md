...
本地网页工具，用来记录交易操作和 TFSA 每次存款与取款。交易记录支持多个账户独立统计，TFSA 页面按年度查看是否接近或超过可用额度。
...
- 默认首页显示交易记录
- 支持新增账户、重命名账户，并在账户之间切换查看
- 每个账户独立记录证券交易、资金流、余额和已实现盈亏
- 支持录入 Bull Call 价差策略，并显示净成本、最大收益、最大亏损和盈亏平衡点
- 单笔交易恢复为卖出匹配买入记录
- Bull Call 价差模式支持匹配已有价差并录入整组平仓
- Bull Call 价差匹配平仓支持无价值到期
- 账户摘要显示已实现收益率，按已实现盈亏除以初始资金余额计算
- 账户摘要显示胜率，按盈利平仓记录数除以已平仓记录数计算
- 录入 TFSA 存款和取款
- 按年份查看历史记录
